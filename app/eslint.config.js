...
export default tseslint.config(
  ...
  forbidLayers("cli", ["domain", "content", "application", "infrastructure", "sim"], ["react", "react-dom"]),

  {
    // The domain is pure: no clock, no ambient randomness, no network. The seed
    // lives in GameState and every shuffle returns the advanced seed. The
    // simulator keeps the same promise, so a run replays from its seed alone.
