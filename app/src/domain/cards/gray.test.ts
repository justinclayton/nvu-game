...
describe("One Man's Junk — 'If any Bad Stuff is played this turn, Oomph 2 and Scramble 2'", () => {
  it("contributes nothing until a piece of Bad Stuff is on the table", () => {
    ...
    const duckC = card("Duck Under");
    const state = playing({
      Gray: player({
        deck: pile("Duck Under", 4),
        hand: [card("One Man's Junk"), duckA, card("Torn Seal"), duckB, duckC],
      }),
    });
    const one = must(state, {
      type: "PLAY_CARD",
      character: "Gray",
      cardId: handCard(state, "Gray", "One Man's Junk").id,
      payWith: [duckA.id],
    });
    ...
    const two = must(one.state, {
      type: "PLAY_CARD",
      character: "Gray",
      cardId: handCard(one.state, "Gray", "Torn Seal").id,
      payWith: [duckB.id, duckC.id],
    });
...
describe("Covering Fire — 'Every time Red plays a card this turn, draw 1 card'", () => {
  it("draws for Gray whenever Red plays, while it is on the table", () => {
    ...
    const once = must(armed.state, free(armed.state, "Red", "Pry Bar"));
    expect(once.state.Gray.hand).toHaveLength(1);
    expect(once.state.Gray.deck).toHaveLength(3);

    const twice = must(once.state, free(once.state, "Red", "Coil Of Cable"));
...
describe("Every Little Bit Helps — 'twice the number of other cards Gray played'", () => {
  it("counts Gray's other played cards", () => {
    ...
    const two = must(one.state, free(one.state, "Gray", "Coil Of Cable"));
...
describe("Level Up — 'Scrap a card from your hand for the top of the Gray Rewards deck'", () => {
  it("Scraps the chosen card and puts the reward straight into hand", () => {
    ...
    const state = playing({
      Gray: player({
        deck: pile("Duck Under", 3),
        hand: [card("Level Up"), duckA, duckB, card("Coil Of Cable")],
      }),
    });
    const top = state.pools.Gray[0];
    if (!top) throw new Error("expected a card in the Gray reward pool");
    const asked = must(state, {
      type: "PLAY_CARD",
      character: "Gray",
    ...
    if (!coil) throw new Error("expected Coil Of Cable among the offered options");
  ...
  it("does nothing if you decline to Scrap", () => {
    ...
    const state = playing({
      Gray: player({
        deck: pile("Duck Under", 3),
        hand: [card("Level Up"), duckA, duckB, duckC],
      }),
    });
    const asked = must(state, {
      type: "PLAY_CARD",
...
describe("I Know Kung Fu — 'Holding: when you play a card with Scramble, draw 1'", () => {
  it("draws while it is held, and not once it is played", () => {
    ...
    if (!firstCoil || !secondCoil) throw new Error("expected two Coils Of Cable in hand");

    // Red playing Scramble does nothing: the card says "when *you* play".
    const byRed = must(state, free(state, "Red", "Coil Of Cable"));
    expect(byRed.state.Gray.deck).toHaveLength(6);

    // Gray playing Scramble while holding it draws.
    const byGray = must(byRed.state, {
      type: "PLAY_CARD",
      character: "Gray",
      cardId: firstCoil.id,
      payWith: [],
    ...
    const silent = must(played.state, {
      type: "PLAY_CARD",
      character: "Gray",
      cardId: secondCoil.id,
      payWith: [],
    });
