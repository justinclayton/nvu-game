...
export const GRAY: Registry = {
   ...
   *
   * §9: no card may be put into a Down character's hand, so with Red out this
   * does nothing. */
  "Here, Catch": {
    onPlay(state, ctx) {
    ...
    onChoice(answer, state, ctx) {
      // §9: nothing may be parked on a Down partner, so the Stuff stays put.
  ...
  /* "Shuffle an exhausted Gray card back into your deck." */
  "Hit 'n Run": {
    onPlay(state, ctx) {
      ...
      return ask(state, {
        kind: "ChooseCards",
        prompt: "Shuffle which exhausted Gray card back into your deck?",
        character: ctx.character,
        options,
        count: 1,
        optional: false,
   ...
   *
   * The Gray Rewards deck is Gray's reward pool (§10). Its top card goes to the
   * hand, not the deck. */
  "Level Up": {
    onPlay(state, ctx) {
