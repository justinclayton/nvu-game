...
describe("Faceful Of Slime — 'Holding: you may not draw more than 1 card'", () => {
  it("caps the draw at one, well short of the usual fill to 5", () => {
    const state = rig({
      phase: "Flip",
...
describe("Spore Cloud — \"Holding: You can't have more than 3 cards in your hand\"", () => {
  it("tightens the hand cap to 3, well short of the usual fill to 5", () => {
    const state = rig({
      phase: "Flip",
      floorDeck: [room("Sorting Room")],
...
describe("My Head Is Quantum Spinning — 'whenever you draw a card, your partner must also draw'", () => {
  /** Red plays Grav Harness on themselves, a clean single draw to react to. */
  function drawViaGravHarness(over: Partial<Parameters<typeof rig>[0]> = {}) {
    const state = rig({
      phase: "Play",
      activeRoom: room("Sorting Room"),
      Red: player({
    ...
    const played = must(state, {
      ...
      payWith: [hand[2] as CardId, hand[3] as CardId],
    });
    return must(played.state, { type: "CHOOSE_CHARACTER", character: "Red" });
  }

  it("forces the partner to draw immediately, and does not chain back", () => {
    const { state: next, events } = drawViaGravHarness();
    // The held card plus Grav Harness's own draw.
    expect(next.Red.hand).toHaveLength(2);
  ...
  it("draws nothing for a partner already holding 5 or more — there is no Full Hand to burn into", () => {
    const { state: next } = drawViaGravHarness({
