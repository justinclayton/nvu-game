...
describe("Crowbar — 'Play: if you get any Good Stuff this turn, get an additional one'", () => {
  it("does nothing when played first, before anything else has paid this turn", () => {
    ...
    const { state: next, events } = play(state, [free(state, "Red", "Crowbar")]);
  ...
  it("takes the extra piece from the room's own payout at Outcome, when it saw no earlier gain", () => {
    ...
    const state = playing({
      ...
      Red: player({
        deck: pile("Shove", 4),
        hand: [card("Crowbar"), shoveA, shoveB],
      }),
    });
    const { state: next, events } = play(state, [
      free(state, "Red", "Crowbar"), // Crowbar, cost 0, nothing to look back at yet
      { type: "PLAY_CARD", character: "Red", cardId: shoveA.id, payWith: [shoveB.id] }, // Shove, Oomph 2
      { type: "END_PLAY" },
    ]);
  ...
  it("does not fire again at Outcome once it already fired on an earlier gain", () => {
    ...
    const { state: next } = play(state, [
      free(state, "Red", "Crowbar"), // Crowbar: fires now, at play, off the look-back
      { type: "PLAY_CARD", character: "Red", cardId: shoveA.id, payWith: [shoveB.id] }, // Shove, Oomph 2
  ...
  it("two Crowbars played by the same controller pay two extra pieces", () => {
    const crowbarA = card("Crowbar");
    const crowbarB = card("Crowbar");
    const shoveA = card("Shove");
    const shoveB = card("Shove");
    const state = playing({
      activeRoom: room("Sorting Room"),
      Red: player({
        deck: pile("Shove", 4),
        hand: [crowbarA, crowbarB, shoveA, shoveB],
      }),
    });
    const { state: next } = play(state, [
      { type: "PLAY_CARD", character: "Red", cardId: crowbarA.id, payWith: [] }, // first Crowbar
      { type: "PLAY_CARD", character: "Red", cardId: crowbarB.id, payWith: [] }, // second Crowbar
      { type: "PLAY_CARD", character: "Red", cardId: shoveA.id, payWith: [shoveB.id] }, // Shove, Oomph 2
      { type: "END_PLAY" },
    ]);
  ...
  it("pays nothing and logs an empty pool when its own bonus draw finds none left", () => {
    ...
    const { state: next, events } = play(state, [
      free(state, "Red", "Crowbar"),
      { type: "PLAY_CARD", character: "Red", cardId: shoveA.id, payWith: [shoveB.id] },
      { type: "END_PLAY" },
    ]);
...
describe("Overcharged Battery — 'The next card played this turn is played for free'", () => {
  it("makes the next card free, and only the next one", () => {
    ...
    // Two identical Charge Ins are left; the discount cares which one comes
    // first, not which one it is, so this reads hand order rather than name.
    const first = charged.state.Red.hand[0];
    const second = charged.state.Red.hand[1];
    if (!first || !second) throw new Error("expected both Charge Ins still in hand");
    expect(costOf(charged.state, "Red", first)).toBe(0);

    const spent = must(charged.state, { type: "PLAY_CARD", character: "Red", cardId: first.id, payWith: [] });
  ...
  it("discounts the next card whichever character plays it", () => {
    ...
    const spent = must(charged.state, free(charged.state, "Gray", "Pick The Lock"));
  ...
  it("is not used up by a card that already costs 0", () => {
    ...
    const played = must(charged.state, free(charged.state, "Red", "Pry Bar"));
...
describe("Rust — 'Holding: Stuff you play has -1 Oomph'", () => {
  it("takes a Oomph off Stuff, and leaves other cards alone", () => {
    ...
    const state = playing({
      Red: player({
        deck: pile("Shove", 3),
        hand: [card("Rust"), card("Pry Bar"), shoveA, shoveB],
      }),
    });
    const withStuff = must(state, free(state, "Red", "Pry Bar"));
  ...
  it("floors a Stuff card's Oomph at zero rather than going negative", () => {
    ...
    const withCard = must(state, free(state, "Red", "Coil Of Cable"));
...
describe("Spore Cloud — 'Holding: At Cleanup, discard cards other than this one until you hold 3'", () => {
  ...
  it("asks its holder to discard down to 3 at Cleanup, Spore Cloud itself not offered", () => {
    ...
    if (!choice) throw new Error("expected a discard option");
    const { state: next, events } = must(asked.state, {
      type: "CHOOSE_CARDS",
      cardIds: [choice.id],
    });
...
describe("Panic — 'ALL rooms require an additional 2 Scramble, and Play: Exhaust 2'", () => {
  it("raises every Scramble threshold while it is held, in either hand", () => {
    ...
    if (!room1) throw new Error("expected Collapsed Stairwell to have a threshold");
  ...
  it("adds a Scramble 2 requirement to an Oomph-only line", () => {
    ...
    if (!line) throw new Error("expected the room to have a threshold");
  ...
  it("does not add a second Scramble floor to a line that is already Scramble", () => {
    ...
    const line = state.activeRoom?.challenges[0]?.thresholds[0];
    if (!line) throw new Error("expected Collapsed Stairwell to have a threshold");
...
describe("My Head Is Quantum Spinning — 'Holding: whenever your partner draws during Play, Exhaust 1'", () => {
  ...
  it("Exhausts 1 when the partner draws during Play", () => {
    ...
    const state = playing({
      Red: player({
        deck: pile("Shove", 4),
        hand: [card("My Head Is Quantum Spinning"), card("Grav Harness"), shoveA, shoveB],
      }),
      Gray: player({ deck: pile("Duck Under", 3) }),
    });
    const asked = must(state, {
      type: "PLAY_CARD",
      character: "Red",
      cardId: handCard(state, "Red", "Grav Harness").id,
      payWith: [shoveA.id, shoveB.id],
    });
    const pending = asked.state.pending;
