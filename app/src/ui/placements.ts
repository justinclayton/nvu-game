...
export function moveDelays(
  ...
  for (const e of events) {
    ...
    // `DISCARD_RESHUFFLED` also has a `cards` field, but it is a count, not a
    // list of cards to place — the reshuffle names no card by id.
    if ("cards" in e && Array.isArray(e.cards)) for (const c of e.cards) note(c.id);
  }
