...
**This ticket was researched twice, independently, by two sessions that did not see each other's
work.** Both passes are preserved below. They were compared on merge and **corroborate each other on
every load-bearing point** — the three-pile definition of Life Force, 1 card = 1 Force, spending and
losing as different verbs with different destinations, the conjunctive loss condition, the
never-reshuffled cycle, drawing out of the resource pool, and the losing player choosing which cards
go. Where they differ it is in depth and evidence base, not in claim. That agreement is itself worth
something: the facts ticket 04 leaned on were reached twice by different routes.

Read **Pass A** for the rules as written, verified against primary PDFs. Read **Pass B** for the
compressed version plus the two findings ticket 04 actually used.

---

## Pass A — primary-source verification
...
## Pass B — original pass, and the source of ticket 04's inputs
