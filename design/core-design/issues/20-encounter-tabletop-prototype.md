...
out, and walk a character through it end to end. Then answer:
...
- The resource model (ticket 04) may not be settled when this first runs. If it is not, stub it
  ...
  findings depend on the stub.

## Provenance

`[proposed by agent → you approved, 2026-08-15]` The map's Notes require feel questions to get
index cards on a real table, but before this ticket the only prototype on the map was ticket 10,
which is numeric. The encounter had no table surface at all.
